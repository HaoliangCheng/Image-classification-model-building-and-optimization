...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        exp_k_kN = torch.exp(sim(z_k, z_k_N) / tau)
        sum_k_kN = 0
        for i in range(2*N):
            if i != k:
               sum_k_kN+= torch.exp(sim(z_k, out[i]) / tau)
        l_k_kN = -torch.log(exp_k_kN/sum_k_kN)
        
        exp_kN_k = torch.exp(sim(z_k_N, z_k) / tau)
        sum_kN_k = 0
        for i in range(2*N):
            if i != k+N:
               sum_kN_k+= torch.exp(sim(z_k_N, out[i]) / tau)
        l_kN_k = -torch.log(exp_kN_k/sum_kN_k)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = out_left.shape[0] 
    pos_pairs = torch.stack([sim(out_left[k], out_right[k]) for k in range(N)]).unsqueeze(1)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def compute_sim_matrix(out):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    sim_matrix = torch.matmul(out, out.t()) / (torch.linalg.norm(out, dim=1) * torch.linalg.norm(out, dim=1).unsqueeze(1))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
