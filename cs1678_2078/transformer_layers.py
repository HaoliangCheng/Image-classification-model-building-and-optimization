...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # 1)
        query = self.query(query).view(N, S, self.n_head, self.head_dim).permute(0, 2, 1, 3)  # (N, S, H, E/H)
        key = self.key(key).view(N, T, self.n_head, self.head_dim).permute(0, 2, 1, 3)  # (N, T, H, E/H)
        value = self.value(value).view(N, T, self.n_head, self.head_dim).permute(0, 2, 1, 3)  # (N, T, H, E/H)
        # 2)
        attn_scores = torch.matmul(query, key.permute(0, 1, 3, 2)) / math.sqrt(self.head_dim)  # (N, H, S, T)
        # 3)
        ...
        # Concatenate and project back to the original embedding dimension
        output = output.permute(0, 2, 1, 3).contiguous().view(N, S, self.emd_dim)  # (N, S, E)
