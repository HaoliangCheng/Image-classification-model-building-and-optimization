...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        visual_features = self.visual_projection(features).unsqueeze(1)  # (N, 1, W)
        # 2)
        tgt_mask = torch.tril(torch.ones(T, T)).unsqueeze(0).to(captions.device)  # (1, T, T)
        # 3)
        # decoder_input = torch.cat((visual_features, caption_embeddings), dim=1)  # (N, T+1, W)
        transformer_output = self.transformer(tgt=caption_embeddings, memory=visual_features, tgt_mask=tgt_mask)  # (N, T+1, W)
